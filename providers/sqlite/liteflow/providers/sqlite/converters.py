...
def dump_workflow_instance(source: WorkflowInstance):
    result = {
        ...
        'version': pickle.dumps(source.version),
        ...
        'execution_pointers': pickle.dumps([dump_execution_pointer(x) for x in source.execution_pointers])
    }
...
def load_workflow_instance(source: Dict) -> WorkflowInstance:
    ...
    result.version = pickle.loads(source['version'])
    result.description = source['description']
    result.data = pickle.loads(source['data'])
    result.next_execution = source['next_execution']
    result.status = int(source['status'])
    result.create_time = source['create_time']
    result.complete_time = source['complete_time']
    result.execution_pointers = [load_execution_pointer(
        x) for x in pickle.loads(source['execution_pointers'])]
